@echo off
chcp 65001 >nul
title Fetch JP/US price history
cd /d "%~dp0"
setlocal EnableExtensions

if exist ".venv\Scripts\python.exe" (
  set "PY=.venv\Scripts\python.exe"
) else (
  echo Creating .venv ...
  python -m venv .venv
  if errorlevel 1 (
    echo Failed to create .venv
    pause
    exit /b 1
  )
  set "PY=.venv\Scripts\python.exe"
)

"%PY%" -c "import yfinance, pandas, requests, pyarrow" >nul 2>&1
if errorlevel 1 (
  echo Installing packages. This can take a few minutes.
  "%PY%" -m pip install -U pip
  "%PY%" -m pip install -r requirements.txt -r requirements-update.txt
  if errorlevel 1 (
    echo Install failed.
    pause
    exit /b 1
  )
)

echo.
echo Yahoo Finance から過去の日足を取得します。
echo 日本株 / 米株。保存先: history\  （カレンダー用）
echo 取得済みの銘柄はスキップします。最長が既にある場合はすぐ終わります。
echo カレンダーに出すには、終わったあと start.bat を開き直してください。
echo 1年: 市場あたり 20〜60分 目安
echo 最長: 数時間かかることがあります。この窓は閉じないでください。
echo --------------------------------
echo 市場
echo  1  日本株
echo  2  米株
echo  3  日本株と米株
echo  0  終了
echo.
set /p MKT=市場を選んでください: 
if "%MKT%"=="1" (set "MARKET=jp" & goto PERIOD)
if "%MKT%"=="2" (set "MARKET=us" & goto PERIOD)
if "%MKT%"=="3" (set "MARKET=both" & goto PERIOD)
goto END

:PERIOD
echo.
echo 期間
echo  1  1年
echo  2  5年
echo  3  最長（Yahooにある限り。銘柄により20年超）
echo  4  開始日を指定  YYYY-MM-DD
echo  0  終了
echo.
set /p PER=期間を選んでください: 
if "%PER%"=="1" (set "RANGE=--period 1y" & goto RUN)
if "%PER%"=="2" (set "RANGE=--period 5y" & goto RUN)
if "%PER%"=="3" (set "RANGE=--period max" & goto RUN)
if "%PER%"=="4" goto CUSTOM
goto END

:CUSTOM
echo.
set /p START=開始日 YYYY-MM-DD: 
if "%START%"=="" goto END
set "RANGE=--start %START%"
goto RUN

:RUN
echo.
echo 取得開始  %MARKET%  %RANGE% ...
echo.
"%PY%" -u scripts\update_quotes.py --history --market %MARKET% %RANGE% --batch-size 40 --sleep 2.0
if errorlevel 1 (
  echo 取得に失敗しました。
  goto END
)
echo.
echo history\ に保存しました。
echo カレンダーを更新するには start.bat を開き直してください。

:END
echo.
pause
